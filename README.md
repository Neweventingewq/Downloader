...
sudo apt-get install -y \
  ...
  qt6-webengine-dev libqt6webenginecore6 libqt6webenginequick6 \
...
- `build-linux.yml` installs the same apt packages listed in the *Building* section (now including `qt6-webengine-dev`, `libqt6webenginecore6`, `libqt6webenginequick6`, `qml6-module-qtwebengine` and `qml6-module-qtwebchannel` for the Step 19 sign-in flow), configures CMake with Ninja, builds, and asserts the resulting binary exists and is executable. Fast: the whole job typically completes in well under five minutes and acts as the canonical "did I break the build?" gate.
- `build-windows.yml` installs Qt 6.8 via `jurplel/install-qt-action` (with module cache), pulls in MSVC via `ilammy/msvc-dev-cmd`, configures CMake with Ninja + `cl.exe`, builds Release, then runs `windeployqt --qmldir qml --webenginecore --compiler-runtime` against the freshly-built `.exe`. The compiler had to flip from MinGW to MSVC for this branch: Qt does not ship pre-built **QtWebEngine** binaries for MinGW on Windows — WebEngine is only available on the `win64_msvc2022_64` distribution. After build it downloads the latest official `yt-dlp.exe` and an *essentials* ffmpeg build, copies `ffmpeg.exe` / `ffprobe.exe` next to the `.exe`, zips the result, and uploads the artefact (and the raw `dist/` tree). The job retains the artefact for 30 days. `--webenginecore` makes windeployqt drop `QtWebEngineProcess.exe`, `icudtl.dat` and the Chromium locale resources next to the binary so the in-app sign-in window renders at runtime; without it the WebView appears blank.

### Step 19 — Embedded YouTube sign-in (cookie jar) with rate-limit safeguards

- The "danced with a tambourine" cookie story (extract cookies.txt by hand, close Chrome, re-export when YouTube rotates the session) finally gets a real fix. The app now ships an embedded QtWebEngine surface that the user logs into YouTube with **once**, after which every subsequent yt-dlp invocation receives cookies straight from the app's own profile — no DPAPI, no file lock, no manual export.
- **`CookieJar` (`src/CookieJar.{h,cpp}`)** wraps a named, persistent `QWebEngineProfile` rooted at `AppLocalDataLocation/web-profile/` (on Windows: `%LOCALAPPDATA%\Volchay\Volchay-Downloader\web-profile\`, on Linux: `~/.local/share/Volchay/Volchay-Downloader/web-profile/`). The profile uses `ForcePersistentCookies` and a stable storage path, so cookies survive both quitting the app and rebooting the machine — exactly like a real browser. The class subscribes to `QWebEngineCookieStore::cookieAdded`/`cookieRemoved`, maintains its own in-memory cookie list, derives `youtubeLoggedIn` from the presence of any Google auth cookie (`__Secure-3PSID`, `__Secure-1PSID`, `SID`, `__Secure-3PSIDTS`, `__Secure-1PSIDTS`), and exposes `exportYouTubeCookiesNetscape()` and `signOutYouTube()` as Q_INVOKABLE. The Netscape exporter writes a yt-dlp-compatible cookies file at `AppLocalDataLocation/yt-cookies.txt` containing only the YouTube/Google/googlevideo cookies the WebEngine profile actually holds — session cookies are normalised to a 30-day future expiry (yt-dlp's `MozillaCookieJar` drops session cookies on load otherwise), and any cookie whose value happens to contain a literal `\t` is skipped (Netscape format is tab-delimited; an embedded tab would corrupt every subsequent line).
- **`LoginWindow.qml`** is a standalone `Window` (not a modal) with a top warning band, a full-height `WebEngineView` whose `profile` is bound to `cookieJar.profile`, and a footer with a live "Signed in — you can close this window" status and a Close button. The page first loaded is `https://accounts.google.com/v3/signin/identifier?continue=https%3A%2F%2Fwww.youtube.com%2F&service=youtube&hl=en` so Google takes the user straight to the YouTube-flavoured sign-in form. Popups from Google's "Use another device" flow are routed back into the same view via `onNewWindowRequested: request.openIn(web)` — opening child windows would have lost the warning banner that lives at the top.
- **The warning banner is non-dismissible** and intentionally amber-on-amber so it reads as "caution" in every theme: *"Do not use your main Google account. Create a separate account for downloads — that way if YouTube ever flags this session, you'll only lose a throwaway account, not your channel."* This is the only safeguard the app physically *can* enforce against the worst-case "I signed in with my real channel and YouTube banned it" outcome; the rest is rate-limiting (see below).
- **`SettingsPage.qml`** grows a new *YouTube sign-in* card immediately above the Network section. It hosts: the warning banner (repeated), a status pill (green "Signed in" when `cookieJar.youtubeLoggedIn` is true, neutral "Not signed in" otherwise), a *Sign in* / *Sign in again* button that calls `loginWindow.openLogin()`, and a *Sign out* button (visible only when signed in) that calls `cookieJar.signOutYouTube()`. The `LoginWindow` instance itself lives at the page root so it survives layout churn inside the AcrylicCard.
- **`DownloadManager::buildYtDlpArgs`** changes priority order for credentials to **CookieJar → manual cookies.txt → cookies-from-browser → anonymous**. The CookieJar path is preferred because (a) it's fresh on every yt-dlp invocation, (b) it never touches Chrome's DPAPI-encrypted database, (c) it doesn't ask the user to paste a file path. The existing manual-file and browser-profile paths remain as fallbacks for users who deliberately don't want to use the in-app login (or simply haven't yet).
- **Rate-limit safeguards.** The most common way to get a logged-in YouTube account flagged is bursting requests — yt-dlp's anti-bot heuristics fire on download patterns, not single calls. Two changes ship together with the sign-in path:
  - `Settings::politeSleepSec` (new, default `3`, range 0–30) is forwarded to yt-dlp as `--sleep-requests <n> --sleep-interval <n> --max-sleep-interval <n>`. Three seconds is invisible to a human watching a download progress bar but is enough to keep YouTube's per-session counters from tripping, which matters disproportionately for accounts attached to large channels.
  - `Settings::maxConcurrent` defaults to **1** for new installs (was 2). Existing users' QSettings values are preserved by the standard load path, so this is purely the new-install default.
- **Localisation.** `AppLocale.cpp` gains the `set.section.signin`, `set.politeSleep`, `set.politeSleep.hint` and the full `signin.*` family in both English and Ukrainian. No string is hardcoded in QML for the new surface.
- **Main initialisation ordering.** `QtWebEngineQuick::initialize()` must run **before** `QGuiApplication` is constructed: WebEngine installs a Chromium-aware event dispatcher into the QPA layer and any QObject created before that runs against a stale dispatcher. The single `initialize()` call in `src/main.cpp` is therefore literally the first executable line in `main`, immediately after the qInfo banner.
- **Build / packaging impact.** CMakeLists adds `WebEngineQuick` and `WebEngineCore` to `find_package(Qt6)` and the corresponding `Qt6::WebEngineQuick` / `Qt6::WebEngineCore` to `target_link_libraries`. The Windows artefact grows by ~50–60 MB (mostly Chromium binaries shipped by `windeployqt --webenginecore`). This is the deliberate trade — the cost of "log in once, never dance with cookies again" is a one-time download that's still under 200 MB total.
